...
describe('UpdateProductUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        provideProductUseCaseWithCache(UpdateProductUseCase),
        loggerProvider(),
  ...
  it('throws when product not found', async () => {
    ...
    await expect(useCase.execute('no-id', { name: 'X' })).rejects.toBeInstanceOf(ProductNotFoundError);
  ...
  it('throws when renaming to a name that already exists on another product', async () => {
    ...
    await expect(useCase.execute(p.id, { name: 'Taken' })).rejects.toBeInstanceOf(ProductAlreadyExistsError);
